...
/// token so a published link is just `notes.parfait.to/<token>` — the GitHub
...
/// the address bar. The notes.parfait.to Worker (workers/notes-proxy) decodes
...
/// Parfait always publishes `meeting.html`; the Worker reattaches it.
enum GistLinkToken {
    /// The gist filename Parfait always publishes; constant, so it's dropped from
