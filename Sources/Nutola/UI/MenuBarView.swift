...
struct MenuBarView: View {
    ...
    @State private var showDailyDigest = false
    @State private var dailyDigest: DailyDigest?
    private let digestGenerator = DailyDigestGenerator()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                let todayAgenda = app.calendar.upcomingDays(limit: 1).flatMap(\.events)
                let summaries = Dictionary(uniqueKeysWithValues:
                    app.store.meetings.map { ($0.id, app.store.summary(for: $0.id)) })
                dailyDigest = digestGenerator.generate(
                    for: .now,
                ...
                Label("Daily Digest", systemImage: "list.bullet.clipboard")
                    .font(.nutola(12, .medium))
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
            .buttonStyle(.plain)
            .help("View yesterday's digest and today's agenda")
            upcoming
            recent
            Divider()
            if showQuitConfirm {
        ...
        .sheet(isPresented: $showDailyDigest) {
            if let digest = dailyDigest {
                DailyDigestSheet(digest: digest) { showDailyDigest = false }
            } else {
...
/// Small sheet displaying the morning briefing generated by `DailyDigestGenerator`.
private struct DailyDigestSheet: View {
